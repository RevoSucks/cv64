...
void FireSparkles_Init(FireSparkles* self) {
    ...
    if (effect_init((Effect*) self)) {
        (*effect_initParams)(
            (Effect*) self, EFFECT_ID_FIRE_SPARKLES, ptr_effectMgr->display_camera, 0
        );
    }

    visual_info        = self->visual_info;
    visual_info->model = self->model = model = effect_getModel((Effect*) self);

    if (!TRUE) {
        // Trivial condition needed for matching
        return;
    }
    ...
    effect_updateGraphicParams((Effect*) self);
    ...
    if (BITS_HAS(visual_info->flags, EFF_VISUAL_FLAG_NO_VARIABLE_TRANSPARENCY)) {
        BITS_UNSET(visual_info->anim_info.visual_flags, EFF_ANIM_VISUAL_FLAG_VARIABLE_TRANSPARENCY);
    }

    effect_createPointLight((Effect*) self);
...
void FireSparkles_Loop(FireSparkles* self) {
    s32 should_despawn = effect_loop((Effect*) self);
